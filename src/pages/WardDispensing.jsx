...
export default function WardDispensing() {
  ...
  const handleSubmit = async () => {
    ...
    if (!encounter) { setError('Patient has no active encounter — cannot issue to ward'); return; }
    ...
    try {
      ...
      setSuccess(`Ward issue ${bill.billNumber} recorded — charged to patient encounter`);
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
                ...
                <div className="card-body" style={{ padding: 14 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
                    <div>
                      ...
                      <div style={{ fontWeight: 600 }}>{pending.drugName}</div>
                        ...
                        <select value={pending.batch?.id ?? ''} onChange={e => {
                          ...
                          {pendingBatches.map(b => (
                            <option key={b.id} value={b.id}>
                              {b.batchNumber} · {new Date(b.expiryDate).toLocaleDateString('en-IN', { month: 'short', year: '2-digit' })} · ₹{fmt(b.sellingPrice)}
                            </option>
                          ))}
                        </select>
                      )}
                    ...
                    <div>
                      ...
                      <input type="number" min="1" value={pending.qty}
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={handleAddToCart}
                        ...
                        className="btn btn-success" style={{ padding: '6px 12px' }}>Add</button>
                      <button onClick={() => { setPending(null); setPendingBatches([]); }}
                        className="btn btn-secondary" style={{ padding: '6px 12px' }}>×</button>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {cart.map(item => {
                    ...
                    return (
                      <div key={item.id} style={{
                        display: 'grid', gridTemplateColumns: '2fr 0.6fr 0.6fr 0.8fr auto',
                        ...
                        <div style={{ textAlign: 'right', fontSize: 12 }}>{item.gstRate}% GST</div>
          ...
          <div>
            <div className="card card-elevated" style={{ position: 'sticky', top: 20 }}>
              <div className="card-header"><h3 style={{ margin: 0, fontSize: 15 }}>Patient & Encounter</h3></div>
              <div className="card-body">
                ...
                <div style={{
                  ...
                  This will be charged to the patient's HMS bill — no cash collected.
                </div>

                <button onClick={handleSubmit}
                  disabled={loading || !cart.length || !encounter}
                  className="btn btn-success btn-block"
                  style={{ fontSize: 13, fontWeight: 700 }}>
                  {loading ? 'Processing…' : `Issue to Ward  ₹${fmt(totals.total)}`}
                </button>
              </div>
            </div>
          </div>
        ...
        {completedIssues.length > 0 && (
          <div style={{ marginTop: 24 }} className="card card-elevated">
            <div className="card-header"><h3 style={{ margin: 0, fontSize: 15 }}>Recent ward issues</h3></div>
            <table className="table">
              <thead>
                <tr><th>Issue #</th><th>Patient</th><th>Ward</th><th style={{ textAlign: 'right' }}>Amount</th><th>Time</th></tr>
              </thead>
              <tbody>
